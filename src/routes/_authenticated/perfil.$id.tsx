            ...
            <div className="grid items-start gap-5 lg:grid-cols-[18rem_minmax(0,1fr)]">
              <aside className="space-y-4 lg:sticky lg:top-24">
                <section className="surface-card p-5">
                  ...
                  <p className="mt-3 text-sm leading-relaxed text-muted-foreground">{profile.about_me || profile.bio || "Esta pessoa ainda não compartilhou detalhes sobre sua jornada."}</p>
                  {interests.length ? <div className="mt-4 flex flex-wrap gap-2">{interests.slice(0, 4).map((interest) => <span key={interest} className="rounded-full bg-primary/10 px-2.5 py-1 text-xs font-semibold text-primary">#{interest}</span>)}</div> : null}
                  <button type="button" onClick={() => setActiveTab("about")} className="mt-4 w-full rounded-lg bg-secondary px-3 py-2 text-sm font-semibold hover:bg-primary/10 hover:text-primary">Ver detalhes</button>
                </section>
                <section className="surface-card p-5">
                  <div className="flex items-center justify-between"><h2 className="text-lg font-bold">Comunidades</h2><span className="text-sm text-muted-foreground">{activity?.communities.length ?? 0}</span></div>
                  <div className="mt-3 space-y-3">{activity?.communities.slice(0, 3).map((community) => community ? <Link key={community.id} to="/comunidades/$slug" params={{ slug: community.slug }} className="flex items-center gap-3 rounded-lg p-1 hover:bg-secondary"><div className="flex size-9 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-primary/10 font-bold text-primary">{community.avatar_url ? <img src={community.avatar_url} alt="" className="size-full object-cover" /> : community.name.charAt(0)}</div><span className="line-clamp-1 text-sm font-semibold">{community.name}</span></Link> : null)}</div>
                  <button type="button" onClick={() => setActiveTab("communities")} className="mt-4 w-full rounded-lg bg-secondary px-3 py-2 text-sm font-semibold hover:bg-primary/10 hover:text-primary">Ver todas</button>
                </section>
              </aside>
              <div className="space-y-4">
                <div className="surface-card flex items-center justify-between p-4"><h2 className="text-lg font-bold">Publicações</h2><span className="text-sm text-muted-foreground">{activity?.posts.length ?? 0} visíveis</span></div>
