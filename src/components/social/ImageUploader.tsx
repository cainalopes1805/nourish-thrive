...
export function ImageUploader({ type, currentUrl, onUploadComplete, className }: ImageUploaderProps) {
  ...
  return (
    <div className={cn("relative group overflow-hidden rounded-lg border-2 border-dashed border-border bg-muted/30 hover:bg-muted/50 transition-colors", className)}>
      {currentUrl ? (
        ...
        <div className="flex flex-col items-center justify-center p-4 text-center h-full min-h-[120px]">
          ...
          <p className="text-sm font-medium">Adicionar {type === 'avatar' ? 'foto' : 'capa'}</p>
          <p className="text-[10px] text-muted-foreground mt-1 mb-3">JPEG/PNG/WEBP até {MAX_SIZE_MB}MB</p>
          <Button 
            variant="outline" 
            size="sm"
            onClick={() => inputRef.current?.click()}
